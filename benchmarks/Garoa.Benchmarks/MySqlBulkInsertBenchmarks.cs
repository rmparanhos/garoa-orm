...
/// <summary>
/// Bulk insert over a real MySQL connection, comparing the two ways to write the same rows to the
/// same table:
/// <list type="bullet">
...
/// </list>
/// The bulk-copy path beats the multi-row INSERT because it skips per-statement SQL parsing,
/// streams the rows, and never materialises a giant statement — so it is both faster and far
/// lighter on allocations. The CI regression gate tracks <c>GaroaBulk</c> against this <c>Dapper</c>
/// baseline.
/// <para>
