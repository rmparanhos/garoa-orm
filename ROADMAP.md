...
- [ ] Single `BulkInsert<T>` interface exposed to the caller, provider implementation
  internal. Streams row-by-row; never builds a giant SQL string.
- [ ] PostgreSQL provider via Npgsql COPY protocol (`NpgsqlBinaryImporter`).
- [ ] MySQL provider via `MySqlBulkCopy`.
- Both require only normal INSERT privileges — no DDL / special grants.

### Connection pool & timeout

- [~] Correct pool management — no connection leaks.
...
- [x] Unit tests for the mapper (incl. DateOnly/TimeOnly, null handling, enums, wrong-
  column error message) + end-to-end tests over SQLite.
- [x] CI: build + test on push and PR (`.github/workflows/ci.yml`).
- [x] CI/CD: **publish the NuGet package** on a `v*.*.*` tag
  (`.github/workflows/publish.yml`; requires the `NUGET_API_KEY` secret).
